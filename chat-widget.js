...
(function() {
    ...
    widgetStyles.textContent = `
        ...
        .chat-assist-widget .chat-header-titles {
            display: flex;
            flex-direction: column;
            gap: 2px;
        }
        ...
        .chat-assist-widget .chat-header-main-title {
            font-size: 18px;
            font-weight: 700;
            color: white;
            line-height: 1.1;
        ...
        <div class="chat-header">
            <div class="chat-header-left">
                ...
                <div class="chat-header-titles">
                    <span class="chat-header-main-title">Support</span>
                    <span class="chat-header-subtitle">Promovista Ai</span>
                </div>
            </div>
    ...
    // Skip registration and start chat directly
    async function skipRegistrationAndStartChat() {
        chatWelcome.style.display = 'none';
        
        // Initialize conversation with default user data
        conversationId = createSessionId();
        const defaultName = 'Guest';
        const defaultEmail = 'guest@example.com';
        
        // Hide registration form, show chat interface directly
        ...
        try {
            ...
            // Display welcome message with clickable links
            const botMessage = document.createElement('div');
            botMessage.className = 'chat-bubble bot-bubble';
            let messageText = "Welcome to the chat! How can I help you today?";
            
            if (sessionResponseData && (Array.isArray(sessionResponseData) ? sessionResponseData[0].output : sessionResponseData.output)) {
                messageText = Array.isArray(sessionResponseData) ? 
    ...
    async function submitMessage(messageText) {
        ...
        const email = nameInput && nameInput.value.trim() ? nameInput.value.trim() : "guest@example.com";
        const name = emailInput && emailInput.value.trim() ? emailInput.value.trim() : "Guest";
    ...
    // Event listeners
    startChatButton.addEventListener('click', skipRegistrationAndStartChat);
    ...
    launchButton.addEventListener('click', () => {
        ...
        // Skip welcome screen and registration, go straight to chat
        chatWelcome.style.display = 'none';
        userRegistration.classList.remove('active');
        
        // Initialize chat if not already initialized
        if (!conversationId) {
            skipRegistrationAndStartChat();
        } else if (!chatBody.classList.contains('active')) {
            chatBody.classList.add('active');
        }
    });
